...
async def cluster_all_dangerous_tweets(
    limit: int = 500,
    min_cluster_size: int = 5,
    min_samples: Optional[int] = 2,
):
    cursor = (
        processed_collection.find({"is_dangerous": True})
        .sort("tagged_at", -1)
        .limit(limit)
    )
    ...
    if len(tweets) < min_cluster_size:
        return {
            ...
            "message": "Not enough dangerous tweets for HDBSCAN clustering.",
            "clusters": [],
            "points": [],
        }
